...
from utils import listAllFilesIn
import config
...
class Obstacles(MyTurtle):
    def __init__(self, x,y):
        super().__init__()
        self.shape(config.OBS_SHAPE)
        self.color('black')
        self.jump(x,y)
        self.vert_limit = 23
        self.honz_limit = 160
...
class Dot(MyTurtle):
    ...
    def obstacles(self,obstcl):
        if self.xcor() < obstcl.xcor() +obstcl.honz_limit and self.xcor() > obstcl.xcor() - obstcl.honz_limit and self.ycor() < obstcl.ycor() + obstcl.vert_limit and self.ycor() > obstcl.ycor():
            self.dead = True
            self.color('red')
    ...
    def verif(self):
        self.bounds()
        self.obstacles(mid_bar)
        self.obstacles(right_bar)
        self.obstacles(down_bar)
        self.find()
        if self.dead == True:
            self.destroy()
...
class Behavior():
    ...
    def randomize(self):
        for i in range(self.decisions):
            self.moves.append([20, randint(-self.turn_freedom, Behavior.turn_freedom)])
...
my_candy = Candy()
mid_bar = Obstacles(180,-70)
right_bar = Obstacles(-180,-70)
down_bar = Obstacles(0,70)
